...
const InfoCard = (props) => {
    const { Info_container, vector, gradientCircle, item, dynamic_padding ,cardrow} = style;
        ...
        <ScrollTrigger onEnter={() => setCounter(true)} onExit={() => setCounter(false)}>
            {
                windowWidth < 580 ? <div className={`${props?.data?.length <= 4 ? " mt-0 mb-5" : `${Info_container} mt-5 `} p-1  p-sm-0  mx-4 ${Info_container}`}>
                                ...
                                <>
                                    ...
                                    >
                                        <div className='ml-n3 text-center' style={{ color: "#3D336E", }}>
                                            <h5 className={`   ${props?.data?.length <= 4 ? 'my-1' : ''}`}>{heading}</h5>
                    ...
                    <div className={`d-flex  ${props?.data?.length <= 4 ? `${dynamic_padding} px-5 justify-content-between` : 'flex-wrap justify-content-center'} align-items-center pt-4`}>
                        {props?.data?.map(({ image, heading, discription }, index) => {
                            const updatedDescription = parseInt(discription.replace(/[K+]/g, ''), 10);
                            return (
                                <div key={index} className={`${item} ${props?.data?.length <= 4 ? '' : 'col-6 col-md-4 col-lg-2'} d-flex flex-column flex-lg-row align-items-center justify-content-center mb-3 text-center text-lg-left`}>
                                    <div className={`ml-n3 ${cardrow}`}>
                                        {!props.count && <h5 className='text-center' style={{ fontSize: 62, fontWeight: "500", color: "#3D336E", fontFamily: 'Barlow, sans-serif' }}>{discription}</h5>}
                                        {props.count && counter &&
